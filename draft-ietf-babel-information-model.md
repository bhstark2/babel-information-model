...
is focused on reporting status of the Babel protocol, and very little of
that is considered mandatory to implement (conditional on a management protocol
...
in angle brackets, \<m..n>, where m indicates the minimal number of values,
and n is the maximum.  The symbol \* for n means no upper bound.
...
Following is a list of the data elements that an implementation can choose
to allow to be configurable:
...
babel-enable:
: if true, the babel implementation is running; if false, the babel
  implementation is not currently running; MAY  be configurable to
  allow babel to be started or stopped
...
: set of values of supported link types; valid enumeration values are
  identified in Babel Link Types registry (see {{iana-considerations}})
...
: a set of names of supported cost computation algorithms; possible
  values include "k-out-of-j", "ETX"
...
babel-routes:
: a set of babel-route-obj objects; includes received and routes
  routes
...
: UDP port for sending and listening for Babel messages; default
  is 6696; MAY be configurable

babel-mcast-group:
: multicast group for sending and listening to multicast
  announcements on IPv6; default is ff02:0:0:0:0:0:1:6; MAY be configurable
...
: if true, babel sends and receives messages on this
  interface; if false, babel messages received on this interface are ignored
  and none are sent; MAY be configurable
...
: external input to cost of link of this interface; if
  ...
  over this interface; how an implementation uses the value is up to the implementation,
  which means the use may not be consistent across implementations; MUST be
  configurable if implemented

babel-message-log-enable:
: if true, logging of babel messages received on
  this interface is enabled; if false, babel messages are not logged; MUST
  be configurable, if implemented

babel-message-log:
: log entries that have timestamp of a received Babel message
  and the entire received Babel message  (including Ethernet frame and IP headers,
  if possible); an implementation must restrict the size of this log, but how
  and what size is implementation-specific; if this log is implemented, a mechanism
...
: the multicast Hello history of whether or not
  the multicast Hello messages prior to babel-exp-mcast-hello-seqno were received;
...
: the unicast Hello history of whether or not the
  unicast Hello messages prior to babel-exp-mcast-hello-seqno were received;
  representation of a binary sequence where the most recently received Hello
  is expressed as  a "1" placed in the left-most bit, with prior bits shifted
  right (and "0" bits placed between prior Hello bits and most recent Hello
  for any not-received Hellos); represented as a string using utf-8 encoded
...
: transmission cost value from the last IHU packet received from
  this neighbor, or maximum value (infinity) to indicates the IHU hold timer
  for this neighbor has expired
...
: if true, the security mechanism is running; if false,
  the security mechanism is not currently running; MAY be configurable to allow
  security mechanism to be started or stopped
...
: a set of babel-credential-obj objects that identify
  the credentials of routers whose babel messages may be trusted or of a certificate
...
: if true, logging of messages that include credentials
  used for authentication is enabled; if false, these messages are not logged;
  MUST be configurable, if implemented
  babel-credvalid-log: log entries that have the timestamp a message containing
  credentials used for peer authentication (e.g., DTLS Server Hello) was received
...
: a boolean flag indicating whether this route is selected,
  i.e., whether it is currently being used for forwarding and is being advertised
...
: the date and time (according to the device internal clock
  setting, which may be a time relative to boot time, acquired from NTP, configured
...
configured. Any mechanism or protocol that is used to transmit this information
or allow for its configuration is also responsible for ensuring this is done
...
security mechanisms that make use of these credentials are not defined or
...
1. Would it be useful to define some parameters for reporting statistics or
  logs? \[2 logs are now included. If others are needed they need to be proposed.]
